...
We are going to understand deletion across `three` different scenarios:

1. `The node to be deleted is a leaf node` - in this case we simply remove the node from the tree

![binary tree](/images/bst_21.png)

2. `The node to be deleted has one child node` - in this case, remove the node and replace it with its child

![binary tree](/images/bst_22.png)

3. `The node to be deleted has two child nodes` - in this case, we move to the right child and keep going left. In the example below, to remove `3` we move to its right child node `6` and go left till there's no more left to go, and replace `3` with the smallest `value` left to `6`

![binary tree](/images/bst_24.png)
...
bst.delete(5)
bst.delete(7)
console.log(bst.inOrder()) // [ 3, 10, 15 ]
